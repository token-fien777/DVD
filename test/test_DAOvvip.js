...
const DAOvvipUpgradeable = artifacts.require("DAOvvipUpgradeable");
const DAOmineUpgradeable = artifacts.require("DAOmineUpgradeable");
...
contract("DAOvvip", async () => {
    ...
    beforeEach(async () => {
        await deployments.fixture(["hardhat_DAOmine_DAOvvip"]);

        const daoVvipUpgradeableProxy = await ethers.getContract("DAOvvipUpgradeableProxy")
        daoVvip = new ethers.Contract(daoVvipUpgradeableProxy.address, DAOvvipUpgradeable.abi, deployer);

        const daoMineUpgradeableProxy = await ethers.getContract("DAOmineUpgradeableProxy")
        daoMine = new ethers.Contract(daoMineUpgradeableProxy.address, DAOmineUpgradeable.abi, deployer);
        
        await dvd.connect(dvdOwner).transferOwnership(daoMine.address);
    });

    describe('initial value', () => {
        it("Should be set with correct initial vaule in DAOvvip", async () => {
            expect(await daoVvip.owner()).equal(deployer.address);
            assert.equal(await daoVvip.dvd(), dvd.address, "The DVD address disagreement");
            assert.equal(await daoVvip.daoMine(), daoMine.address, "The DAOvvip address disagreement");
            assert.equal(await daoVvip.poolId(), 1, "pool ID is incorrect");

            for (let i = 0; i < network_.DAOvvip.tierAmounts.length; i++) {
                assert.equal(await daoVvip.tierAmounts(i), network_.DAOvvip.tierAmounts[i], `tierAmounts(${i}) is incorrect`);
            }
        });

        it("Should be set with correct initial vaule in DAOmine", async () => {
            ...
            assert.equal(await daoMine.daoVvip(), daoVvip.address, "The DAOvvip address disagreement");
            assert.equal(await daoMine.daoVvipPid(), 1, "daoVvipPid is incorrect");
            assert.equal(await daoMine.poolLength(), 2, "xDVD and DAOvvip pools are not added by default");
            assert.equal(await daoMine.totalPoolWeight(), network_.DAOmine.xdvdPoolWeight * 2, "total pools weight is incorrect");
            ...
            assert.equal(await daoMine.earlyHarvestPenaltyPeriod(), network_.DAOvvip.earlyHarvestPenaltyPeriod, "earlyHarvestPenaltyPeriod is incorrect");
            assert.equal(await daoMine.earlyHarvestPenaltyPercent(), network_.DAOvvip.earlyHarvestPenaltyPercent, "earlyHarvestPenaltyPercent is incorrect");
            ...
            for (let i = 0; i < network_.DAOvvip.lockDays.length; i++) {
                assert.equal(await daoMine.lockDays(i), network_.DAOvvip.lockDays[i], `lockDays(${i}) is incorrect`);
            }
            for (let i = 0; i < network_.DAOvvip.lockBonusRate.length; i++) {
                assert.equal(await daoMine.lockBonusRate(i), network_.DAOvvip.lockBonusRate[i], `lockBonusRate(${i}) is incorrect`);
            }
